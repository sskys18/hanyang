...
isDrag=0
# new variables
click = 0 # button click 횟수(6번까지 가능)
animStartTime = 0
cow2wlds = []
...
def display():
    global cameraIndex, cow2wld, click, animStartTime, cow2wlds
    ...
    if click < 6:
        ...
        for i in range(click):
            drawCow(cow2wlds[i], cursorOnCowBoundingBox)
    else:
        animTime = glfw.get_time() - animStartTime
        if animTime <= 18:
            t = animTime - int(animTime)

            b_spline = np.array([[-1/6, 1/2, -1/2, 1/6],
                                  [1/2, -1, 1/2, 0],
                                  [-1/2, 0, 1/2, 0],
                                  [1/6, 2/3, 1/6, 0]])


            p_t = np.array([t ** 3, t ** 2, t, 1]) @ b_spline

            q = np.array([cow2wlds[int(animTime + 5) % 6].copy()[:3, 3],
                          cow2wlds[int(animTime) % 6].copy()[:3, 3],
                          cow2wlds[int(animTime + 1) % 6].copy()[:3, 3],
                          cow2wlds[int(animTime + 2) % 6].copy()[:3, 3]])

            cow = np.identity(4)
            cow[:3, 3] = p_t @ q

            x_axis = np.array([3 * (t ** 2), 2 * t, 1, 0]) @ b_spline @ q
            ...
            space = np.array([[x_axis[0], x_axis[1], x_axis[2], 0],
                              [y_axis[0], y_axis[1], y_axis[2], 0],
                              [z_axis[0], z_axis[1], z_axis[2], 0],
                              [0, 0, 0, 1]])

            drawCow(cow @ np.linalg.pinv(space), False)
...
def onMouseButton(window,button, state, mods):
    global isDrag, V_DRAG, H_DRAG, click, cow2wlds, cow2wld, cursorOnCowBoundingBox, animStartTime
    GLFW_DOWN = 1
    GLFW_UP = 0
...
def main():
    ...
    width = 800
    height = 600
    window = glfw.create_window(width, height, '2018008040-PA2', None, None)
    if not window:
        glfw.terminate()
        sys.exit(-1)
