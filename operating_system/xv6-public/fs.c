...
// triple indirect
  ...
  // Load triple indirect block, allocating if necessary.
    ...
    uint idx = bn / TINDIRECT;       // triple indirect 블록 내에서의 인덱스
    uint idx2 = (bn % TINDIRECT) / DINDIRECT;  // double indirect 블록 내에서의 인덱스
    uint idx3 = (bn % TINDIRECT) % DINDIRECT; // single indirect 블록 내에서의 인덱스
